...
export default function Profile() {
    ...
    try {
      ...
      if (data && data.gender) {
        ...
        const safeName = encodeURIComponent(
          data.username || data.full_name || "User"
        );
        ...
        const isAdventurer = currentAvatar.includes(
          "dicebear.com/9.x/adventurer"
        );

        if (
          gender === "male" &&
          (!currentAvatar.includes("hair=short") || !isAdventurer)
        ) {
          ...
          gender === "female" &&
          (!currentAvatar.includes("hair=long") || !isAdventurer)
        ) {
        ...
        if (shouldUpdate) {
          console.log(
            `Fixing avatar mismatch for ${gender}: ${currentAvatar} -> ${newAvatarUrl}`
          );
  ...
  const handleUnblock = async (friendshipId, userName) => {
    try {
      ...
      setBlockedUsers((prev) =>
        prev.filter((u) => u.friendship_id !== friendshipId)
      );
  ...
  const handleAvatarExported = async (event) => {
    try {
      ...
      // Get 2D snapshot URL by replacing extension (RPM feature)
      // Default 2D URL is often just replacing .glb with .png?
      // Actually RPM provides specific params.
      // Better strategy: Use the specialized image API if needed,
      // BUT simple replacement works for standard RPM avatars usually.
      // Let's use the RPM Image API format for better control if needed,
      // but for now simple replacement is robust for standard 'models.readyplayer.me' links.

      // Standard RPM URL: https://models.readyplayer.me/64b...glb
      // Image URL: https://models.readyplayer.me/64b...png
      const pngUrl = glbUrl.replace(
        ".glb",
        ".png?quality=100&size=1024&background=false"
      );

      console.log("Avatar Exported:", glbUrl, pngUrl);

      showToast("Saving new avatar... 🎨");

      // Update Database
      // Add timestamp to force refresh explicitly
      const timestamp = new Date().getTime();
      const glbUrlWithTime = `${glbUrl}?t=${timestamp}`;
      const pngUrlWithTime = `${pngUrl}&t=${timestamp}`;

      console.log("Avatar Exported:", glbUrlWithTime, pngUrlWithTime);

      showToast("Saving new avatar... 🎨");

      // Update Database - Try updating BOTH first
      try {
          const { error: fullUpdateError } = await supabase
            .from("profiles")
            .update({
              avatar_url: pngUrlWithTime,
              avatar_3d_url: glbUrlWithTime,
            })
            .eq("id", user.id);
          ...
          if (fullUpdateError) throw fullUpdateError;
           ...
           // Update Local State
           const updatedUser = { ...user, avatar_url: pngUrlWithTime, avatar_3d_url: glbUrlWithTime };
           setUser(updatedUser);
           setShowAvatarCreator(false);
           showToast("Avatar updated! Looks great! ✨");

      } catch (err) {
          console.warn("Full update failed, trying fallback (missing avatar_3d_url?):", err);

          // Fallback: Update ONLY avatar_url (if avatar_3d_url column missing)
          ...
          if (fallbackError) {
           ...
           // Update Local State (Partial)
           const updatedUser = { ...user, avatar_url: pngUrlWithTime }; // Keep old 3D url or null
           setUser(updatedUser);
           setShowAvatarCreator(false);
           showToast("Avatar updated (2D only) - Check DB schema! ⚠️");
      }
    } catch (error) {
    ...
    <div className="profile-page">
      ...
      <div className="profile-header-card">
        <div className="avatar-wrapper">
          {user.avatar_3d_url ? (
            ...
            <img
              src={(() => {
                ...
                // Fallback to legacy logic if no RPM avatar
                if (user.avatar_url?.includes("readyplayer.me"))
                  return user.avatar_url;

                if (gender === "male")
                  return `https://api.dicebear.com/9.x/adventurer/svg?seed=${safeName}&hair=short01,short02,short03,short04,short05,short06,short07,short08&earringsProbability=0`;
                if (gender === "female")
                  return `https://api.dicebear.com/9.x/adventurer/svg?seed=${safeName}&glassesProbability=0&mustacheProbability=0&beardProbability=0&hair=long01,long02,long03,long04,long05,long10,long12`;
          ...
          {!user.avatar_3d_url && <div className="status-indicator"></div>}
        </div>
        
        {/* Absolute Edit Name Button */}
        <button
        ...
        <div className="profile-info">
          ...
          <div className="tags-row">
            ...
            {/* Pencil Icon for Avatar Edit - Beside Status */}
            <button 
      ...
      <div className="scroll-content">
        ...
        <div className="menu-group">
          <MenuItem
            ...
            label="Notifications"
            value={
              user.mute_settings?.message &&
              user.mute_settings.message !== "Never"
                ? `Muted: ${user.mute_settings.message}`
                : ""
            }
                ...
                {["10 Minutes", "1 Hour", "24 Hours", "Never"].map((dur) => (
                  <button
                    ...
                    className={`chip-option ${
                      user.mute_settings?.message === dur ? "active" : ""
                    }`}
      ...
      {/* Avatar Creator Modal - Full Screen */}
      {showAvatarCreator && (
        <div
          style={{
            position: "fixed",
                  ...
                  <div key={user.friendship_id} className="blocked-user-item">
                    <img
                      src={(() => {
                        ...
                        if (user.gender === "Male")
                          return `https://avatar.iran.liara.run/public/boy?username=${safeName}`;
                        if (user.gender === "Female")
                          return `https://avatar.iran.liara.run/public/girl?username=${safeName}`;
      ...
      <style>{`
                ...
                .profile-info h1 { 
                    margin: 0; font-size: 1.8rem; font-weight: 700; margin-bottom: 4px;
                    color: var(--text-primary);
                ...
                .tags-row { display: flex; gap: 10px; justify-content: center; align-items: center; flex-wrap: wrap; }
                .tag {
